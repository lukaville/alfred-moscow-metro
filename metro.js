...
modules.define("router", ["inherit", "route", "filter"], function(t, Inherit, Route, Filter) {
    var config = {
    ...
    t(Inherit({
        __constructor: function() {
            ...
            this._limit = {},
            this._currRouteId = 0,
            this._filter = new Filter(config.filterConfig)
        },
        _resetLimits: function() {
            this._optimal.time = 1 / 0,
            this._optimal.transfers = config.lineGraphMaxDepth + config.transferToleranceCount,
            this._optimal.hops = 1 / 0,
            this._limit.time = 1 / 0,
            this._limit.transfers = config.lineGraphMaxDepth + config.transferToleranceCount,
        ...
        _updateLimits: function(metrics) {
            metrics.transfers < this._optimal.transfers && (this._optimal.transfers = metrics.transfers,
            0 === this._optimal.transfers && (this._limit.transfers = 0)),
            metrics.time < this._optimal.time && (this._optimal.time = metrics.time,
            this._limit.time = this._optimal.time * config.timeToleranceRatio),
            metrics.hops < this._optimal.hops && (this._optimal.hops = metrics.hops,
            this._limit.hops = this._optimal.hops * config.hopToleranceRatio)
        },
        _recursiveRoutingCall: function(station_from, station_to, route) {
            var routes = [];
            var route = route || new Route;
            route.addStation(station_from);

            if (this._isWithinLimits(route.getMetrics())) {
                if (station_from.getId() === station_to.getId()) {
                    route.setId(this._currRouteId++),
                        this._updateLimits(route.getMetrics()),
                        routes.push(route), routes
                } else {
                    station_from.getLinks().forEach(function (link) {
                        var opposite = link.getOppositeStation(station_from);
                        if (!opposite.isFullyClosed() && !route.isStationVisited(opposite)) {
                            var r = route.clone();
                            r.goThroughLink(link);
                            var a = this._recursiveRoutingCall(opposite, station_to, r);
        ...
        findRoutes: function(route_from, route_to) {
            return this._resetLimits(),
            this._filter.process(this._recursiveRoutingCall(route_from, route_to))
        }
    }))
}),
...
modules.define("filter", ["inherit", "atomic-filters-ns", "route-sorter"], function(t, e, i, routeSorter) {
    t(e({
        __constructor: function(t) {
            var e = {
                sort: (new routeSorter).sort
            };
        ...
        process: function(routes) {
            var result = routes.slice();
            return this._filters.forEach(function(t) {
                if (result.length < 2) {
                    return result
                } else {
                    return void (result = t.process(result))
                }
            }, this),
            result
        },
...
modules.define("check-for-direct-route", ["inherit"], function(t, e) {
    t(e({
        ...
        process: function(routes) {
            var sorted = this._ctx.sort(routes, "transfersTime");
            if (0 === sorted[0].getMetrics().transfers) {
                return [sorted[0]]
            } else {
                return sorted
            }
...
modules.define("remove-double-transfer-routes", ["inherit"], function(t, e) {
    t(e({
        ...
        process: function(t) {
            return t.filter(function(t) {
                if (t.getMetrics().transfers < 2)
                    return !0;
                var e = t.getStations()[0].getId()
                  , i = t.getStations()[0].getLine().getId()
                  , n = [];
                return t.getStations().every(function(t) {
                    var o = t.getLine().getId();
                    if (o !== i) {
                        if (n.push(e),
                        n.length > 1)
                            return t.getLinks().every(function(t) {
                                return t.getFromStation().getId() !== n[0] && t.getToStation().getId() !== n[0]
                            })
                    } else
                        n.length > 0 && (n = []);
                    return i = o,
...
modules.define("genarcho", ["inherit"], function(t, e) {
    var i = e({
        ...
        process: function(t) {
            var e = this._ctx.sort(t, "timeTransfers")
              , i = this._ctx.sort(t, "transfersTime")
              , n = [];
            n.push(e[0].getId()),
            n.push(i[0].getId());
            var o = e[0].getMetrics().time * this._timeToleranceRatio
              , s = e[0].getMetrics().transfers + this._transferToleranceCount;
            e.some(function(t) {
                return t.getId() !== e[0].getId() && t.getId() !== i[0].getId() && t.getMetrics().time <= o && t.getMetrics().transfers <= s ? (n.push(t.getId()),
                !0) : !1
            });
            var r = []
              , a = [];
            return n.forEach(function(e) {
                if (-1 === r.indexOf(e)) {
                    var i;
                    t.some(function(t) {
                        return t.getId() === e && (i = t)
                    }),
                    i && a.push(i),
                    r.push(e)
                }
            }),
            this._ctx.sort(a, "roundedTimeTransfers")
        }
    });
    t(i)
}),
